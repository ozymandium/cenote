...
public:
    ...
    /// with the given ambient absolute pressure. A return value of 1.0 means the controlling compartment would be at
    /// its M value. A return value of 0 means that all compartments would be on-gassing or at equilibrium.
    /// while on-gassing, GF's may reach very large negative values, so 0 is the lowest return value.
