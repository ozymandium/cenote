...
{
    EXPECT_ANY_THROW(Usage(Plan::Point{1_s, 0_m, ""}, Plan::Point{0_s, -1_m, ""}, Water::FRESH));
}
...
TEST(Usage, Surface)
{
    ...
    EXPECT_EQ(Usage(p1, p2, Water::SALT), 10_L);
}
...
TEST(Usage, ConstDepth)
{
    ...
    EXPECT_UNIT_NEAR(Usage(p1, p2, Water::SALT), 20_L, 0.1_L);
}
...
TEST(Usage, AvgDepth)
{
    ...
    EXPECT_UNIT_NEAR(Usage(p1, p2, Water::SALT), 30_L, 0.2_L);
}
